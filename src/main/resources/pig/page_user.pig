...
pig_dsacd_user_group = GROUP pig_dsacd_user_filter BY (page, date1);
pig_dsacd_user_page_unique = FOREACH pig_dsacd_user_group {
	pig_unique_emailid = DISTINCT pig_dsacd_user_filter.emailid;
	GENERATE FLATTEN(group) as (page, date1), COUNT(pig_unique_emailid) as unique_users, COUNT(pig_dsacd_user_filter) as total_hits;
};
pig_dsacd_user_page_unique_order = ORDER pig_dsacd_user_page_unique BY total_hits desc;
pig_dsacd_user_page_unique_order_fin = FOREACH pig_dsacd_user_page_unique_order GENERATE date1, 'cf', page, (total_hits / unique_users) as total_by_uniquser;
