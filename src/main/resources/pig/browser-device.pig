...
pig_dsacd_user_group = GROUP pig_dsacd_user_filter BY (device, browser, gender, date1);
pig_dsacd_user_group_count = FOREACH pig_dsacd_user_group GENERATE FLATTEN(group), COUNT(pig_dsacd_user_filter) as grp_count;
pig_dsacd_user_group_count_concat = FOREACH pig_dsacd_user_group_count GENERATE CONCAT( CONCAT( CONCAT( CONCAT( group::device, '-'), group::browser), '-'), group::gender), 'cf', group::date1, grp_count;
