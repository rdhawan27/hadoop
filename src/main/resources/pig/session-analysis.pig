...
pig_clicks = FOREACH pig_dsacd_user_filter GENERATE sessionid, (long)timestamps, date;
pig_sessions_group = GROUP pig_clicks BY sessionid;
pig_sessionize = FOREACH pig_sessions_group GENERATE MAX(pig_clicks.date) as date, 'cf', group, ( MAX(pig_clicks.timestamps) - MIN(pig_clicks.timestamps)) / 1000.0 / 60 as session_length_min;
--DUMP pig_sessionize;
...
pig_sessionize_avg_median = FOREACH pig_sessiontimes_all {
	...
	GENERATE MAX(pig_ordered.date) as date, FLATTEN(Median(pig_ordered.session_length_min)) as med_session, AVG(pig_ordered.session_length_min) as avg_session;
};
